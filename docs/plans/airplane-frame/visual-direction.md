...
| Motion | Quiet state updates (fade/crossfade; status fields without bounce) **plus** short staged panel settle on load/refresh. **Plus** one-shot hero silhouette arrival: scroll-driven to trailing-edge-in-view (rows L→R, columns bottom→top), hard latch after arrive; past-center panels on rebuild get time-based fly-in after settle (~1s, 110ms stagger). No continuous ambient. Honor `prefers-reduced-motion` (static rest pose). Unsupported scroll-timeline browsers: static rest. Ship: [`css/poster-silhouette-motion.css`](../../../css/poster-silhouette-motion.css) + [`js/silhouette-motion.js`](../../../js/silhouette-motion.js). Probe: [`silhouette-scroll-motion.html`](../../design-mocks/silhouette-scroll-motion.html) |
