...
Non-secret config lives in `worker/wrangler.toml` (`CACHE_TTL_SECONDS`, `STALE_TTL_SECONDS`, `EMPTY_CACHE_TTL_SECONDS`, `MAX_ATTEMPT`, `MAX_AIRLABS`, `MAX_RESULTS`, `PACK_SIZE`). Pi `worker.env` should match (legacy `MAX_ENRICH` still accepted as `MAX_ATTEMPT` fallback).
